            ...
            <form action="{{route('topvideos.update',$top_video->id)}}" enctype="multipart/form-data" method="POST">
                    ...
                    <div class="col-sm-10">
                        <select class="form-control" name="video_type_id" id="video_type_id" autofocus="">
                            <?php foreach ($types as $key => $type): ?>
                                <option <?php if($top_video->video_type_id == $type->id) echo 'selected' ?> value="{{$type->id}}" >{{$type->name}}</option>
                            <?php endforeach ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        ...
                        <input type="file" name="image" id="image"  accept="image/*" id="upload" <?php if(!$top_video->thumbnail) echo 'required'?> >
                        <input type="hidden" name="thumbnail" id="thumbnail" value="{{$top_video->thumbnail}}">
                    </div>
                </div>
